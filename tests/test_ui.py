    ...
    async def test_main_screen_keeps_keyboard_focus_in_prompt(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            async with app.run_test(size=(100, 30)) as pilot:
                ...
                self.assertIn("CONTEXT[/]  0%", str(app.query_one("#session-metrics", Static).renderable))
                self.assertIn("ARTINIUM", str(app.query_one("#artinium-status", Static).renderable))
    ...
    async def test_command_menu_opens_while_generation_is_running(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            async with app.run_test(size=(100, 30)) as pilot:
                ...
                self.assertFalse(app.screen.query_one("#command-permissions", ListItem).disabled)
                await app.screen._rebuild("settings")
                self.assertFalse(app.screen.query_one("#command-model-settings", ListItem).disabled)
                await app.screen._rebuild("context")
                self.assertFalse(app.screen.query_one("#command-model-context", ListItem).disabled)
